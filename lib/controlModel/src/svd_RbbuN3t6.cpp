                  ...
                  [4])
{
  ...
  if (nrm > 0.0F) {
    ...
    b_A[0]++;
    b_s[0] = -nrm;
    xaxpy_QKqL3KGB(-(xdotc_DLfZ4l1Y(b_A, b_A) / b_A[0]), b_A);
  } else {
    b_s[0] = 0.0F;
  }
  ...
  U[3] = 1.0F;
  if (b_s[0] != 0.0F) {
    xaxpy_QKqL3KGB(-(xdotc_DLfZ4l1Y(U, U) / b_A[0]), U);
    U[1] = -U[1];
    U[0] = (-U[0]) + 1.0F;
  } else {
    U[1] = 0.0F;
  ...
  while (static_cast<boolean_T>(static_cast<int32_T>((((m + 2) > 0) ? (
    ...
    while (!(exitg1 || (q == 0))) {
      ...
      if (rt <= ((std::abs(b_s[0]) + std::abs(b_s[1])) * 1.1920929E-7F)) {
        e[0] = 0.0F;
        exitg1 = true;
      } else if (static_cast<boolean_T>(static_cast<int32_T>(((rt <=
          9.86076132E-32F) ? (static_cast<int32_T>(1)) : (static_cast<int32_T>(0)))
        | ((static_cast<boolean_T>(static_cast<int32_T>(((iter > 20) ? (
          static_cast<int32_T>(1)) : (static_cast<int32_T>(0))) & ((rt <=
          (1.1920929E-7F * nrm)) ? (static_cast<int32_T>(1)) :
    ...
    switch (kase) {
     case 1:
      ...
      qs = m + 1;
      while (qs >= (q + 1)) {
        xrotg_arF5SisE(&b_s[0], &rt, &ztest, &sqds);
        xrot_R4zJUYbR(V, 1, ((m + 1) << 1ULL) + 1, ztest, sqds);
        qs = 0;
      }
      break;
     ...
     case 2:
      ...
      e[q - 1] = 0.0F;
      for (qs = q; qs < (m + 2); qs++) {
        xrotg_arF5SisE(&b_s[qs], &rt, &ztest, &sqds);
        rt = (-sqds) * e[qs];
        e[qs] *= ztest;
        xrot_R4zJUYbR(U, (qs << 1ULL) + 1, ((q - 1) << 1ULL) + 1, ztest, sqds);
      }
      break;
     ...
     case 3:
      ...
      if (static_cast<boolean_T>(static_cast<int32_T>(((ztest > sqds) ? (
              static_cast<int32_T>(1)) : (static_cast<int32_T>(0))) | (rtIsNaNF
             (sqds) ? (static_cast<int32_T>(1)) : (static_cast<int32_T>(0))))))
      {
        sqds = ztest;
      }
      ...
      while ((q + 1) <= (m + 1)) {
        xrotg_arF5SisE(&rt, &ztest, &sqds, &smm1);
        rt = (sqds * b_s[0]) + (smm1 * e[0]);
        e[0] = (sqds * e[0]) - (smm1 * b_s[0]);
        ztest = smm1 * b_s[1];
        b_s[1] *= sqds;
        xrot_R4zJUYbR(V, 1, 3, sqds, smm1);
        xrotg_arF5SisE(&rt, &ztest, &sqds, &smm1);
        b_s[0] = rt;
     ...
     default:
      if (b_s[q] < 0.0F) {
        b_s[q] = -b_s[q];
        xscal_gXiGqFhg(-1.0F, V, (q << 1ULL) + 1);
      }
      ...
      while (((q + 1) < 2) && (b_s[q] < b_s[iter])) {
        ...
        b_s[q] = b_s[iter];
        b_s[iter] = rt;
        xswap_PQB3sQhy(V, (q << 1ULL) + 1, ((q + 1) << 1ULL) + 1);
        xswap_PQB3sQhy(U, (q << 1ULL) + 1, ((q + 1) << 1ULL) + 1);
        q = iter;
        iter++;
      }
